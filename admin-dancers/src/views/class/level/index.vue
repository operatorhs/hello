  ...
  <div class="lt-20">
    <el-form ref="searchForm" class="search-form" :model="searchForm" :inline="true" label-width="80px">
      <el-form-item label="等级名称">
        <el-input v-model="searchForm.name" placeholder="请输入等级名称" size="mini" clearable />
      </el-form-item>
      <el-form-item>
...
export default {
  ...
  methods: {
    ...
    _getLists() {
      ...
      reqClassLevelList(this.searchForm).then(res => {
    ...
    sumbmitForm(form) {
      ...
      if (this.isEdit) {
        ...
        reqClassLevelUpload(form, form.assessmentLevelID).then(res => {
          tips('修改成功')
        })
      } else {
        form.createTime = new Date()
        reqClassLevelAdd(form).then(res => {
          tips('添加成功')
        })
      }
    },
    ...
    handleEdit(item) {
      this.tableLoading = true
      reqClassLevelByID(item.assessmentLevelID).then(res => {
        this.tableLoading = false
        this.editForm = res
        this.isEdit = true
        this.handlEdialogEdit()
    ...
    handleDelete(item) {
      this.tableLoading = true
      this.$confirm('此操作将永久删除该考核等级, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        reqClassLevelDetele(item.assessmentLevelID).then(res => {
          this._getLists()
          this.$message({
            type: 'success',
            message: '删除成功!'
