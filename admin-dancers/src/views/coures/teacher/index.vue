...
export default {
  ...
  methods: {
    ...
    _getLists() {
      ...
      reqLecturerList(this.searchForm).then(res => {
    ...
    sumbmitForm(form) {
      ...
      if (this.isEdit) {
        ...
        reqLecturerUpload(form, form.courseLecturerID).then(res => {
          tips('修改成功')
        })
      } else {
        form.createTime = new Date()
        reqLecturerAdd(form).then(res => {
          tips('添加成功')
        })
      }
    },
    handleEdit(item) {
      this.tableLoading = true
      reqLecturerByID(item.courseLecturerID).then(res => {
        this.tableLoading = false
        this.editForm = res
        this.isEdit = true
        this.handlEdialogEdit()
    ...
    handleDelete(item) {
      ...
      this.$confirm('此操作将永久删除该讲师, 是否继续?', '提示', {
        ...
        reqLecturerDetele(item.courseLecturerID).then(res => {
          this._getLists()
          this.$message({
            type: 'success',
            message: '删除成功!'
